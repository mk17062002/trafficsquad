...
if (isset($_POST['vdetail'])) {
	...
	$uid=$_SESSION['uid'];
	$date=date("Y-m-d");
	$sql="INSERT INTO `vehicle_detail` (`vid`, `uid`, `vname`, `vhno`, `vtype`, `vmodel`, `pid`, `ccost`, `date`) 
	VALUES (NULL, '$uid', '$vname', '$vno', '$vtype', '$vmodel', '$pid', '0', '$date')";
	$run=mysqli_query($con,$sql);
	if($run){
	...
	while($row=mysqli_fetch_array($run)){
       ...
       $challancost=$rows['ccost'];
        echo "  <div class='row'>
          ...
          <div class='col-md-2'>
            <h3 class='text-center text-white'>$vhno</h3>
          </div>
          <div class='col-md-2'>
